...
class Waiting implements Event {

    private final Customer customer;
    private final Server server;
    private final double time;
    private static final String TYPE = "Waiting";

    Waiting(Customer customer, double time) {
        this.customer = customer;
        this.time = time;
        this.server = new Server(0);
    }

    Waiting(Customer customer, double time, Server server) {
        this.customer = customer;
        this.time = time;
        this.server = server;
    }
        ...
        Optional<Event> serve = Optional.empty();
            for (int i = 0; i < servers.size(); i++) {
                ...
                if (servers.get(i).getID() == this.server.getID()) {
                    ...
                    serve = Optional.of(new WaitServe(customer, time, server));
                    break;
                }
            } 
